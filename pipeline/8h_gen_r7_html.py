...
FNAME=f"美國總統E-H MONEY R7 ({STAMP}).html"
...
<title>美國總統 E/H MONEY R7 — 三引擎整合 A∩B∩C（{STAMP}）</title>
<style>{CSS}</style>
</head><body>
<button class="theme-btn" id="theme-btn" type="button">☀️ 淺色模式</button>
<header class="hero">
  <div class="hero-eyebrow">R7 · 三引擎整合 A∩B∩C · 現任錨定 · 生成 {esc(HK.strftime('%Y-%m-%d %H:%M'))} 香港時間</div>
  <h1>美國總統 E/H MONEY <span class="hero-r1">R7</span></h1>
...
<footer class="foot">
  <p>版本：R7（三引擎整合 A∩B∩C · 現任錨定 · 月份刻度 · 三 PANE）· 生成 {esc(HK.strftime('%Y-%m-%d %H:%M'))} 香港時間 · 檔名 {esc(FNAME)}</p>
...
for f in glob.glob(os.path.join(ROOT,"美國總統E-H MONEY R7 (*).html")): os.remove(f)
