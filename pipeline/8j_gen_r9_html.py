...
JS =open(os.path.join(ROOT,"assets","r9_engine.js")).read()
A  =json.load(open(f"{S}/anchor_r9.json"))
...
FNAME=f"美國總統E-H MONEY R9 ({STAMP}).html"
...
<title>美國總統 E/H MONEY R9 — 三引擎整合 A∩B∩C（{STAMP}）</title>
<style>{CSS}
{R8CSS}</style>
</head><body>
...
<header class="hero r8-hero">
  <div class="r8-hero-row">
    <h1>美國總統 E/H MONEY <span class="hero-r1">R9</span></h1>
  ...
  <b>PANE 1 新增「重要節點(&gt;60分)」圖層</b>——把《總統週期100節點評分表》中重要性分數 &gt;60 的 45 個結構性節點畫成三角標記
  （紅=決定級≥85／橙=高70-85／藍=中60-70），與現屆轉區旗標、事件標籤分層獨立顯示。
  R9 · 生成 {esc(HK.strftime('%Y-%m-%d %H:%M'))} 香港時間。</p>
</header>
...
<footer class="foot">
  <p>版本：R9（三引擎整合 A∩B∩C · 現任錨定 · 月份刻度 · 三 PANE）· 生成 {esc(HK.strftime('%Y-%m-%d %H:%M'))} 香港時間 · 檔名 {esc(FNAME)}</p>
...
for f in glob.glob(os.path.join(ROOT,"美國總統E-H MONEY R9 (*).html")): os.remove(f)
