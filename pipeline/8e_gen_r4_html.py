...
JS =open(os.path.join(ROOT,"assets","r4_engine.js")).read()
A  =json.load(open(f"{S}/anchor_combined.json"))
...
FNAME=f"美國總統E-H MONEY R4 ({STAMP}).html"
...
<title>美國總統 E/H MONEY R4 — 三引擎整合 A∩B∩C（{STAMP}）</title>
<style>{CSS}</style>
</head><body>
<header class="hero">
  <div class="hero-eyebrow">R4 · 三引擎整合 A∩B∩C · 現任錨定 · 生成 {esc(HK.strftime('%Y-%m-%d %H:%M'))} 香港時間</div>
  <h1>美國總統 E/H MONEY <span class="hero-r1">R4</span></h1>
...
<section id="pane1" class="block">
  ...
  <div class="cy-block">
    <div class="cy-head"><div>
      ...
      <p class="cy-sub"><b style="color:#fff">粗白線＝現屆整合指數</b>（A∩B∩C 共識，21 日平滑）；
...
<section id="pane3" class="block">
  ...
  <p class="muted"><b style="color:var(--unc)">注意：左側色條標示所用標準</b>——
...
<footer class="foot">
  <p>版本：R4（三引擎整合 A∩B∩C · 現任錨定 · 月份刻度 · 三 PANE）· 生成 {esc(HK.strftime('%Y-%m-%d %H:%M'))} 香港時間 · 檔名 {esc(FNAME)}</p>
...
for f in glob.glob(os.path.join(ROOT,"美國總統E-H MONEY R4 (*).html")): os.remove(f)
